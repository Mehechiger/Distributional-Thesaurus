...
class AzDist:
    ...
    def simsCal(self):
        def CalCos():
            ...
            for cat in self.cats:
                # convert weighted word vectors into pandas' DataFrame format and fill missing data with 0
                ...
                # simCos = numerator / denominator
                #        = numerator x denominator(reciprocal)
                simsCos = np.multiply(wVecsDot, normsOuter)
                # convert into DataFrame and restore keys
                simsCos = pd.DataFrame(simsCos, wVecsKs, wVecsKs)
                # append result
                sims[cat] = simsCos
            return sims

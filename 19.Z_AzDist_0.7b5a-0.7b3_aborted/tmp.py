    ...
    def simsCal(self):
        def CalCos():
            ...
            for cat in tqdm(self.cats):
                ...
                sims[cat] = simsCos
                """
                sims[cat] = (wVecsDot, normsOuter, wVecsKs)
            res = {}
            lab = ""
            for we in self.cats:
                res[we] = (sims[we][0], sims[we][1], [sims[we][2], ])
                lab = we
                break
            for cat in self.cats:
                if cat == lab:
                ...
                delta = np.zeros((n, m))
                beta = np.zeros((m, n))
                lat0 = np.hstack((lab0, delta))
                del lab0
                gc.collect()
                cat0 = np.hstack((beta, cat0))
                lat0 = np.vstack((lat0, cat0))
                del cat0
                gc.collect()
                lat1 = np.hstack((lab1, delta))
                del lab1, delta
                gc.collect()
                cat1 = np.hstack((beta, cat1))
                del beta
                gc.collect()
                lat1 = np.vstack((lat1, cat1))
                del cat1
                gc.collect()
                lab2.append(cat2)
                del cat2
                ...
                del lab2
                gc.collect()
                res[lab] = (lat0, lat1, lat2)

            a, b = res[lab][0:2]
            ress = ne.evaluate("a*b")
            print(ress)
            return {}
            return sims

...
def clock(func):  # function timer, as decorator
...
class AzDist:
    ...
    def __init__(self, corpsDir, PoSs, mode, wFunc, sFunc, size, path):
        ...
        self.PoSs, self.wFunc, self.sFunc, self.size, self.path = PoSs, wFunc, sFunc, int(
            size), path
    ...
    # output format:
    # {PoS1: (word vectors in 2DArray — "values", word list — "keys"),
    #  PoS2: (word vectors in 2DArray — "values", word list — "keys"),
    #        ...}
    ...
    def simsCal(self):
        ...
        def Jaccard():
            ...
            for PoS in self.PoSs:
                ...
                simsPoS = sd.pdist(wVecs.T, lambda u, v: np.where(
                    u < v, u, v).sum()/np.where(u > v, u, v).sum())
                simsPoS = sd.squareform(simsPoS)
                sims[PoS] = (simsPoS, wVecsKs)
        ...
        def Lin():
            ...
            for PoS in self.PoSs:
                ...
                simsPoS = sd.pdist(wVecs.T, lambda u, v: (
                    np.where(v != 0, u, 0)+np.where(u != 0, v, 0)).sum()/(u.sum()+v.sum()))
                simsPoS = sd.squareform(simsPoS)
                sims[PoS] = (simsPoS, wVecsKs)
    ...
    # and write them in separate csv files
    def thesGen(self):
        ...
        for PoS in self.PoSs:
            ...
            thesPoS = pd.DataFrame(thesPoS)
            # overwrite alert to be added
            thesPoS.to_csv(path_or_buf=self.path+"AzDist_"+PoS+".csv")

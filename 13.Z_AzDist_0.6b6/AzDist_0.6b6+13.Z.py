...
class AzDist:
    ...
    def cttsExtract(self):
        ...
        for cttsCATk, cttsCATv in tqdm(ctts.items()):
            ...
            for k, v in cttsCATv.items():
                if v['WORD_COUNT'] > 10:  # word count threshold
                    tmp = {kk: vv for kk, vv in v.items() if kk !=
                           'WORD_COUNT' and vv > 5}  # context count threshold
                    # the utility of this line remains to be seen
                    # perhaps it is needed in the weighting part
                    # if it is unneccessary, deleting it could allow us to pass some
                    # checks in the simsCal part and thus gain speed
    ...
    def simsCal(self):
        ...
        for wVecsCATk, wVecsCATv in self.wVecs.items():  # k, v for key and value
            # CaledPairs stocks the simScores of all pairs already calculated
            simsCAT, CaledPairs = {}, {}
            ...
            for wVec1k in tqdm(wVecsCATv.keys()):
                ...
                for wVec2k in wVecsCATv.keys():  # 2 for 2nd word in a pair
                    ...
                    try:  # again, the try/except trick
                        ...
                        pass
                    # initialize 3 variables, where
                    # sumXiYi for dot product of 2 vectors x and y
                    # sumXiPow, sumYiPow for sum of the square of each component of a vector x or y
                    ...
                    for k2, v2 in wVec2.items():
                        if k2 != "WORD_COUNT":
                            sumYiPow += v2*v2
                    # cosine equation
                    # for some reason the denominator can happen to be 0, try/except to avoid error
                    try:
                        simsWord[wVec2k] = sumXiYi / \
                            (math.sqrt(sumXiPow*sumYiPow))
                    except:
                        simsWord[wVec2k] = 0
                    # stock the simScore of each calculed pair
